# Importing libraries
import streamlit as st
import pandas as pd
import matplotlib.pyplot as plt


# Function to shorten category names based on cutoff
def shorten_categories(categories, cutoff):
    categorical_map = {}
    for i in range(len(categories)):
        if categories.values[i] >= cutoff:
            categorical_map[categories.index[i]] = categories.index[i]
        else:
            categorical_map[categories.index[i]] = "Other"
    return categorical_map


# Clean experience values
def clean_experience(x):
    if x == "More than 50 years":
        return 50
    if x == "Less than 1 year":
        return 0.5
    return float(x)


# Clean education values
def clean_education(x):
    if "Bachelor’s degree" in x:
        return "Bachelor’s degree"
    if "Master’s degree" in x:
        return "Master’s degree"
    if "Professional degree" in x or "Other doctoral" in x:
        return "Post grad"
    return "Less than a Bachelors"


# Load and clean data
@st.cache_data
def load_data():
    df = pd.read_csv("Data/survey_results_public.csv")
    df = df[["Country", "EdLevel", "YearsCodePro", "Employment", "ConvertedComp"]]
    df = df[df["ConvertedComp"].notnull()]
    df = df.dropna()
    df = df[df["Employment"] == "Employed full-time"]
    df = df.drop("Employment", axis=1)

    country_map = shorten_categories(df.Country.value_counts(), 400)
    df["Country"] = df["Country"].map(country_map)

    # Filter salary range and remove 'Other'
    df = df[(df["ConvertedComp"] <= 250000) & (df["ConvertedComp"] >= 10000)]
    df = df[df["Country"] != "Other"]

    # Apply cleaning functions
    df["YearsCodePro"] = df["YearsCodePro"].apply(clean_experience)
    df["EdLevel"] = df["EdLevel"].apply(clean_education)

    # Rename column for clarity
    df = df.rename({"ConvertedComp": "Salary"}, axis=1)
    return df


df = load_data()


# Display explore page
def show_explore_page():
    st.title("📊 Explore Software Engineer Salaries")
    st.markdown("### Insights from Stack Overflow Developer Survey")

    # Pie chart - Number of responses by country
    st.subheader("🌍 Number of Responses from Different Countries")
    country_data = df["Country"].value_counts()
    fig1, ax1 = plt.subplots()
    ax1.pie(
        country_data,
        labels=country_data.index,
        autopct="%1.1f%%",
        shadow=True,
        startangle=90,
    )
    ax1.axis("equal")  # Ensure pie is a circle
    st.pyplot(fig1)

    st.markdown("---")

    # Bar chart - Mean Salary by Country
    st.subheader("💵 Mean Salary Based on Country")
    country_salary = df.groupby("Country")["Salary"].mean().sort_values(ascending=True)
    st.bar_chart(country_salary)

    st.markdown("---")

    # Line chart - Mean Salary by Years of Experience
    st.subheader("📈 Mean Salary Based on Years of Experience")
    exp_salary = df.groupby("YearsCodePro")["Salary"].mean().sort_values(ascending=True)
    st.line_chart(exp_salary)
